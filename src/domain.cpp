...
namespace hexwatershed
{
  ...
  int domain::domain_setup_model()
  {
    ...
    mParameter.insert(std::pair<std::string, std::string>(
                                                          "dLatitude_top",
                                                          convert_double_to_string(dLatitude_top)));
    mParameter.insert(std::pair<std::string, std::string>(
                                                          "dLatitude_bottom",
                                                          convert_double_to_string(dLatitude_bottom)));
    mParameter.insert(std::pair<std::string, std::string>(
                                                          "dLongitude_left",
                                                          convert_double_to_string(dLongitude_left)));
    mParameter.insert(std::pair<std::string, std::string>(
                                                          "dLongitude_right",
                                                          convert_double_to_string(dLongitude_right)));
  ...
  int domain::domain_retrieve_user_input()
  {
    ...
    if (search != mParameter.end())
      {
        this->dLatitude_top = std::stod(search->second);
      }
    sKey = "dLatitude_bottom";
    search = mParameter.find(sKey);
    if (search != mParameter.end())
      {
        this->dLatitude_bottom = std::stod(search->second);
      }
    sKey = "dLongitude_left";
    search = mParameter.find(sKey);
    if (search != mParameter.end())
      {
        this->dLongitude_left = std::stod(search->second);
      }
    sKey = "dLongitude_right";
    search = mParameter.find(sKey);
    if (search != mParameter.end())
      {
        this->dLongitude_right = std::stod(search->second);
      }
  ...
  int domain::read_hexagon_polygon_netcdf(std::string sFilename_hexagon_netcdf_in)
  {
    int error_code = 1 ;
    int iDimension;
    size_t lDimension ;
